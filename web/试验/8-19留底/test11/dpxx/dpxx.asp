...
<script>
	...
	function gettree(js)
	{
		$.post('dpxx_json_all_get.asp?cmd=tree_get',function(result){
			treeys(result.商铺信息);
		},'json');
	}
	...
	function initTree(){
		$('#tree_ys').tree({
			onClick: function(node){
				var id=node.id;
				$('#dg').datagrid({ url:'dpxx_json_all_get.asp?cmd=get',queryParams:{ shop_id:id } });
			}
		});
	}
	...
	function rows(){
		var rowsl= 0 ;
		$('#dg').datagrid({ url:'dpxx_json_all_get.asp?cmd=get',queryParams:{ rows:rowsl } });
	}

	//查找按钮事件
	function doSearch(){
		var mcc = $("#mc").textbox('getValue');
		$('#dg').datagrid({ url:'dpxx_json_all_get.asp?cmd=get',queryParams:{ mc:mcc } });
	}
	...
	function remove(){
		...
		if (row) {
			...
			//信息提示确认信息
				if (r){
					$.post('dpxx_json_del.asp?cmd=del',{Shop_ID:row.Shop_ID},function(res){
						$('#dg').datagrid('reload');
					},'json');
	...
	function qiyong(){
		...
		if (row) {
			...
			//信息提示确认信息
				if (r){
					$.post('dpxx_json_del.asp?cmd=enable',{Shop_ID:row.Shop_ID},function(res){
						$('#dg').datagrid('reload');
	...
	function jingyong() {
		...
		if (row) {
			...
			//信息提示确认信息
				if (r){
					$.post('dpxx_json_del.asp?cmd=unenable',{Shop_ID:row.Shop_ID},function(res){
						$('#dg').datagrid('reload');
						if(res.code!=1){ tsshow(res.msg) };
					},'json');
				//如果是r（确定）提交删除地址url，指定选中行的id，结果提示信息，传输方法为json
	...
	//下拉框
	function getbox(){
		$.post('dpxx_json_all_get.asp?cmd=lx_get',function(res){
			$('#LX').combobox({
			valueField:'Shop_ID',
    		textField:'Shop_MC',
    		data:res.rows
